...
const unitLedgerCache: Record<
  string,
  {
    ...
    summary: {
      ...
      totalExpense: number;
      balance: number;
      cateringKitchenExpense?: number;
      cateringOfficeExpense?: number;
    };
  }
> = {};
  ...
  const [summary, setSummary] = useState(
    () =>
      cached?.summary || {
        ...
        totalExpense: 0,
        balance: 0,
        cateringKitchenExpense: 0,
        cateringOfficeExpense: 0,
      }
  );
    ...
    try {
      ...
      if (res.ok) {
        ...
        const smm = {
          ...
          cateringKitchenExpense: Number(json.summary?.cateringKitchenExpense || 0),
          cateringOfficeExpense: Number(json.summary?.cateringOfficeExpense || 0),
        };
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-3 border-b border-slate-200">
        ...
        {/* Action Buttons Ringkas & Fungsional */}
        <div className="flex flex-wrap items-center gap-2">
          <a
            href={`/api/export/excel?type=transaksi&businessUnit=${unit}`}
            className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 text-xs font-semibold shadow-subtle transition-all"
          ...
          <Link
            ...
            title="Catat uang keluar / belanja operasional"
          >
          ...
          <Link
            ...
            title="Catat uang masuk / pendapatan"
          >
      ...
      {/* KPI Cards Ringkasan Finansial Unit */}
      {unit === 'CATERING' ? (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          {/* Card 1: Total Pemasukan Catering */}
          <div className="bg-white rounded-2xl p-4 border border-slate-200/90 shadow-subtle flex flex-col justify-between">
            ...
            <div className="mt-2">
              <div className="text-lg sm:text-xl font-bold tracking-tight text-emerald-700 tabular-nums">
                +{formatRupiah(summary.totalIncome)}
              </div>
            </div>
          </div>

          {/* Card 2: Biaya Pokok Dapur */}
          <div className="bg-white rounded-2xl p-4 border border-slate-200/90 shadow-subtle flex flex-col justify-between">
            <div className="flex items-center justify-between text-xs font-medium text-slate-500">
              <span>Biaya Pokok Dapur</span>
              <div className="w-7 h-7 rounded-lg bg-rose-50 text-rose-700 flex items-center justify-center border border-rose-100">
                <ArrowUpRight className="w-4 h-4" />
              </div>
            </div>
            <div className="mt-2">
              <div className="text-lg sm:text-xl font-bold tracking-tight text-rose-700 tabular-nums">
                -{formatRupiah(summary.cateringKitchenExpense || (summary.totalExpense - (summary.cateringOfficeExpense || 0)))}
              </div>
            </div>
          </div>

          {/* Card 3: Operasional Kantor Catering */}
          <div className="bg-white rounded-2xl p-4 border border-slate-200/90 shadow-subtle flex flex-col justify-between">
            <div className="flex items-center justify-between text-xs font-medium text-slate-500">
            ...
            <div className="mt-2">
              <div className="text-lg sm:text-xl font-bold tracking-tight text-amber-700 tabular-nums">
                -{formatRupiah(summary.cateringOfficeExpense || 0)}
              </div>
            </div>
          </div>

          {/* Card 4: Laba Bersih Catering */}
          <div className="bg-white rounded-2xl p-4 border border-slate-200/90 shadow-subtle flex flex-col justify-between">
            <div className="flex items-center justify-between text-xs font-medium text-slate-500">
        ...
        /* 3 KPI Cards Ringkasan Finansial Unit Umum */
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-600">
              <thead className="bg-slate-50 text-slate-700 font-semibold border-b border-slate-200">
                <tr>
                  <th className="py-3 px-3.5 w-28 whitespace-nowrap">Tanggal</th>
                  <th className="py-3 px-3.5 w-44">Pos Akun</th>
                  <th className="py-3 px-3.5">Keterangan</th>
                  <th className="py-3 px-2.5 w-16 text-center">Metode</th>
                  <th className="py-3 px-3.5 w-28 text-right whitespace-nowrap">Uang Masuk</th>
                  <th className="py-3 px-3.5 w-28 text-right whitespace-nowrap">Uang Keluar</th>
                  <th className="py-3 px-2.5 w-16 text-center">Aksi</th>
                </tr>
              </thead>
                    ...
                    <tr key={tx.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="py-3 px-3.5">
                        <div className="flex items-start gap-1.5">
                          {tx.account && (
                            <span className="px-1.5 py-0.5 rounded bg-slate-100 border border-slate-200 text-[10px] font-mono font-semibold text-slate-600 shrink-0">
                              {tx.account.code}
                            </span>
                          )}
                          <span className="font-semibold text-slate-900 text-xs">
                            {tx.account?.name || tx.category}
                          </span>
                        </div>
                        {tx.account && tx.category && tx.category !== tx.account.name && (
                      ...
                      <td className="py-3 px-3.5 text-slate-700 max-w-sm break-words">
                        {tx.description}
                      </td>
                      ...
                      <td className="py-3 px-3.5 text-right font-bold tabular-nums whitespace-nowrap text-emerald-700">
                        {isIncome ? `+${formatRupiah(Number(tx.amount))}` : '-'}
                      </td>

                      <td className="py-3 px-3.5 text-right font-bold tabular-nums whitespace-nowrap text-rose-600">
                        {!isIncome ? `-${formatRupiah(Number(tx.amount))}` : '-'}
                      </td>
